        ...
        <div id="messagecenter_all">
           ...
           <!--未读消息  -->
             ...
             <%
		        	 ...
		        	 // List<MessageBean> notRAReviewsmessages = uService.findnotReadAReviewsMessages(uid);//未读“文章”评论消息
		        		for(int i=0;i<notRReviewsmessages.size();i++){
		        			...
		        			MessageBean mBean = notRReviewsmessages.get(i);
            ...
            <table id="messagecenter_table" border="1">
                ...
                <%
	            	List<MessageBean> RReviewsmessages = uService.findReadReviewsMessagesByUid(uid);
	            	if(RReviewsmessages.isEmpty()){
           		%>
                <tr>
                    <td colspan="4" style="padding: 20px;"><strong>没有消息记录！</strong></td>                     
                </tr>
                <%
                		...
                		//following_id :关注者， follower_id: 被关注者
  		        	  //接受消息者:receiver_id  发送消息者:answer_id
  		        		for(int i=0;i<RReviewsmessages.size();i++){
  		        			int number=i+1;
  		        			request.setAttribute("number", number);
  		        			MessageBean mBean = RReviewsmessages.get(i);
  		        			request.setAttribute("mBean", mBean);
  		        			String sendMUserId = mBean.getAnswer_id();//发消息用户id
  		        			UserBean uBean = uService.findUserInfo(sendMUserId);
  		        			request.setAttribute("uBean", uBean);
